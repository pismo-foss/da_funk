...
module PosxmlParser
  class Download
    ...
    def riak_mapreduce_request(socket, company_name, file_name, crc, current_app, logical_number)
      ...
      downloaded  = @first_packet.length

      return -3 if (partial_download_to_store(file_name, response_size, file_size, downloaded) < 0)
    ...
    def partial_download_to_store(file_name, response_size, file_size, downloaded)
      begin
        ...
        if (response_size > 1024)
          ...
          while(last_downloaded != downloaded && downloaded < (response_size - 1))
    ...
    def mount_request
      ...
      @request[0..3] = [@request.size].pack("N")
    end
